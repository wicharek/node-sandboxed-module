...
SandboxedModule.prototype._createRecursiveRequireProxy = function() {
  ...
  if((!this._options.sourceTransformersSingleOnly && this._options.sourceTransformers) ||
      this._options.requireFilter ||
      this._options.fakeModules){
    options = {
      sourceTransformers: this._options.sourceTransformers,
      requireFilter: this._options.requireFilter,
      fakeModules: this._options.fakeModules
    };
  }
  ...
  function RecursiveRequireProxy(request){
    ...
    if (this._options.fakeModules && this._options.fakeModules[request]) {
      return this._options.fakeModules[request];
    }
    ...
    // do not use sandbox require proxy for files that do not match the filter
    var requestedFilename = requireLike(this.filename).resolve(request);
    var useProxy = this._options.requireFilter ? this._options.requireFilter(requestedFilename) : true;
    ...
    if (useProxy) {
      ...
      return require(requestedFilename);
    }
  }
